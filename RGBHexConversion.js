...
function rgb(r, g, b) {
  ...
  const calcHex = (color) => {
    color = Math.max(0, Math.min(255, color)); // Math.max retorna o maior valor e Math.min o menor
    let hex = "";
  ...
  return colors.map(calcHex).join(""); // map chama a função com todos os valores de colors
}
